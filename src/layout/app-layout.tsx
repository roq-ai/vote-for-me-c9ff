...
import { getParties } from 'apiSdk/parties';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['election-campaign-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<PartyInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/parties` : null),
    () => getParties({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Party',
      path: isTenantUser ? `/parties/view/${data?.[0]?.id}` : '/parties',
      entity: 'party',
