...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getPartyById();
    case 'PUT':
      return updatePartyById();
    case 'DELETE':
      return deletePartyById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getPartyById() {
    const data = await prisma.party.findFirst(convertQueryToPrismaUtil(req.query, 'party'));
  ...
  async function deletePartyById() {
